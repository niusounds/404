...
def main_script_runner():
    system_prompt = (
        ...
        "1. 最初の行で必ずタイトルを # 見出し形式で記述してください。\n"
        "2. 導入（平和な日常や学校の描写）→ 展開（異常な出来事や恐怖の体験）→ 結末（戦慄のオチ）を持つ、物語を記述してください。\n"
        "3. 物語に「制服」「廊下」「部活動」「忘れ物」「購買」など、学校的な要素を複数組み込んでください。\n"
        "4. 装飾や余計な前置きは一切不要です。本文のみを生成してください。"
    )
